...
module.exports = {
  ...
  get_mempoolinfo: function(cb) {
  	if(settings.use_rpc) {
  		rpcCommand([{method:'getmempoolinfo', parameters: []}], function(response) {
  			return cb(response);
  		});
  	}
  },
  ...
  get_rawmempool: function(cb) {
  	if(settings.use_rpc) {
  	  // set "verbose" parameter to "true" for all mempool tx details
  	  rpcCommand([{method:'getrawmempool', parameters: [true]}], function(response) {
  		  return cb(response);
  	  });
  	}
  },
  ...
  get_blockstats: function(height, cb) {
...
*/
  ...
  get_block_fees: function(height, cb) {
    Tx.find({'blockindex':height}, 'fee').exec(function(err, docs) {
      var block_fees = 0;
      module.exports.syncLoop(docs.length, function(loop) {
        var i = loop.iteration();
        block_fees += docs[i].fee;
        loop.next();
      }, function() {
        // micropresident: It doesn’t decide what to burn, but what to keep. So it should keep 112 (out of 225 sats)
        var block_fees_burned_sats = Math.round(block_fees / 2);
        // return burned block fees in XPI units
        var block_fees_burned_xpi = parseFloat(block_fees_burned_sats / 1000000).toFixed(6);
        return cb(block_fees, block_fees_burned_xpi);
      });
    });
  },

  // return burned supply in XPI
  get_burned_supply: function(cb) {
    Block.find({}, 'burned').where('burned').gt(0).exec(function(err, docs) {
      // total network fees burned, in satoshis
      var burned_supply = 0;
      module.exports.syncLoop(docs.length, function(loop) {
        var i = loop.iteration();
        // micropresident: It doesn’t decide what to burn, but what to keep. So it should keep 112 (out of 225 sats)
        burned_supply += docs[i].burned;
        loop.next();
      }, function() {
        return cb(parseFloat(burned_supply).toFixed(6));
      });
    });
  },
  ...
  is_block_orphaned: function(height, cb) {
    module.exports.get_blockhash(height, function(hash) {
      module.exports.get_block(hash, function(block) {
        // if confirmations is -1, then this block is orphaned!
        if(block.confirmations === -1) {
        // check previous block height too to make sure no orphan
          return module.exports.is_block_orphaned(height--, cb);
        } else {
  ...
  calculate_fee: function(vout, vin, cb) {
  	module.exports.calculate_total(vout, function(vout_total) {
  		module.exports.calculate_total(vin, function(vin_total) {
  			//return cb(parseFloat((vin_total - vout_total) / size).toFixed(3));
  			return cb(vin_total - vout_total)
  		});
  	});
  },
  ...
  prepare_vout: function(vout, txid, vin, cb) {
    ...
    module.exports.syncLoop(vout.length, function (loop) {
      ...
      // if this is an OP_RETURN output, we want to add output value as burned supply
      } else if (vout[i].scriptPubKey.type == 'nulldata' && vout[i].value > 0) {
        burned += vout[i].value;
        module.exports.convert_to_satoshi(parseFloat(vout[i].value), function(amount_sat){
          // also add to the vout array in order for lib.calculate_fee to work properly
          // in database.js:save_tx(), we ignore op_return outputs for address balance updates
          arr_vout.push({addresses: "OP_RETURN " + vout[i].n, amount: amount_sat, asm: vout[i].scriptPubKey.asm });
          loop.next();
        });
      } else {
      ...
      if (vout[0].scriptPubKey.type == 'nonstandard') {
        if ( arr_vin.length > 0 && arr_vout.length > 0 ) {
          if (arr_vin[0].addresses == arr_vout[0].addresses) {
            ...
            arr_vin.shift();
            return cb(arr_vout, arr_vin, burned);
          } else {
            return cb(arr_vout, arr_vin, burned);
          }
        } else {
          ...
          module.exports.is_unique(arr_vin, addresses[0].hash, function(unique, index) {
            if (unique == true) {
              module.exports.convert_to_satoshi(parseFloat(addresses[0].amount), function(amount_sat){
                arr_vin.push({addresses:addresses[0].hash, amount:amount_sat, num_inputs:1});
                loop.next();
              });
            } else {
              module.exports.convert_to_satoshi(parseFloat(addresses[0].amount), function(amount_sat){
