...
app.use('/ext/getburnedsupply', function(req, res) {
    ...
    if (tx) {
        ...
        if(block.confirmations === -1) {
    	    lib.get_blockhash(block.height, function(replacement_block) {
    	    	res.send({error: 'tx orphaned by block ' + replacement_block});
    	    });
      ...
      lib.get_rawtransaction(txid, function(rtx) {
        if (rtx.txid) {
          ...
          lib.get_block(rtx.blockhash, function(block) {
            ...
            if(block.confirmations === -1) {	
              lib.get_blockhash(block.height, function(replacement_block) {
    	    	  res.send({error: "tx orphaned by block", blockhash:replacement_block});
    	        });
                  ...
                  if (!rtx.confirmations > 0) {
                    lib.get_rawmempool(function(rawmempool) {
                      ...
                      if(rawmempool[rtx.txid] === undefined) {
                        res.send({error:"tx not in mempool",txid:rtx.txid});
                      }
                      var utx = {
                        txid: rtx.txid,
                        size: rtx.size,
...
app.use('/ext/getlastblocksajax', function(req, res) {
  ...
  db.get_last_blocks_ajax(req.query.start, req.query.length, function(blocks, count) {
    ...
    for(i = 0; i < blocks.length; i++) {
      var block = blocks[i];
      var row = [];
      row.push(block.height);
      row.push(block.minedby);
      row.push(block.size);
      //row.push(block.fees);
      row.push(block.txcount);
      row.push(block.burned);
      row.push(new Date((block.timestamp) * 1000).toUTCString());
      data.push(row);
    }
    */
