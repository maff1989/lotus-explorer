        ...
        if (err) {
          ...
          db.check_stats(settings.coin, function(exists) {
            if (exists == false) {
              ...
              // get coin current stats
              db.get_stats(settings.coin, function(stats){
                // get Lotus current block count
                lib.get_blockcount(function(count) {
                
                  if (settings.heavy == true) {
                    db.update_heavy(settings.coin, count, 20, function(){
                  ...
                  if (mode == 'reindex') {
                    Tx.deleteMany({}, function(err) { 
                      ...
                      Address.deleteMany({}, function(err2) { 
                        ...
                        AddressTx.deleteMany({}, function(err3) {
                          ...
                          Block.deleteMany({}, function(err4) {
                              ...
                              db.update_tx_db(settings.coin, 1, count, settings.update_timeout, function(){
                                db.update_richlist('received', function(){
                                  db.update_richlist('balance', function(){
                                    db.update_db(settings.coin, function(nstats){
                    ...
                    db.update_tx_db(settings.coin, 1, count, settings.check_timeout, function(){
                      db.update_db(settings.coin, function(nstats){
                    ...
                    // start at next block, don't reiterate over most recent saved block (stats.last + 1)
                    db.update_tx_db(settings.coin, stats.last + 1, count, settings.update_timeout, function(){
                      console.log("updating charts db...")
                      db.update_charts_db(function() {
                        db.update_richlist('received', function(){
                          db.update_richlist('balance', function(){
                            db.update_db(settings.coin, function(nstats){
                    ...
                    db.update_tx_db(settings.coin, stats.last + 1, count, settings.check_timeout, function(){
